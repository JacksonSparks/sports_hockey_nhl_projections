...
def scrape_box_score(box_score_url):
    try:
        ...
        first_period_goals = [row.find_all('td')[2].text.strip() for row in rows]
...
def main():
    ...
    if all_game_data:
        save_to_csv('nhl_sp_gamelog.csv', all_game_data)
        print("Scraped data saved to nhl_sp_gamelog.csv")
    else:
